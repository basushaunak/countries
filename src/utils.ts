...
async function hydrateCountryMaps(countries: Country[]): Promise<Country[]> {
  return Promise.all(
    countries.map(async (country) => {
      ...
      const blob = await fetchCountryMap(
        country.name,
        "APIKEY"
      );
...
/**
 * Fetch a static map image for a country from LocationIQ
...
async function fetchCountryMap(
  ...
  await delay(1000);
  const { width = 600, height = 400, zoom = 4, format = "jpg" } = options;

  if (!countryName.trim()) {
    throw new Error("countryName must be a non-empty string");
  }
  ...
  const encodedLocation: string = encodeURIComponent(countryName);
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch map for "${countryName}" (${response.status} ${response.statusText})`
    );
  }
